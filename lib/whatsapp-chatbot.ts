...
async function lookupByPhone(phone: string) {
  ...
  return prisma.booking.findMany({
    where: {
      OR: [
        { customerWhatsapp: { contains: clean.slice(-9) } },
      ],
    },
...
Customers book at *www.umrahtransport.me* and pay by card. They receive a booking reference (e.g. UT-XXXXX or HUR-XXXXX). A driver is automatically assigned after payment.

## Your Capabilities

1. **Answer questions** about services, pricing, vehicles, coverage areas
2. **Check booking status** — if customer provides a booking reference, look it up
3. **Help with changes** — explain what they need to do (direct to website or say you'll flag it to the team)
4. **General Umrah advice** — basic travel tips, what to expect, prayer times guidance
5. **Complaints** — acknowledge, apologise sincerely, assure follow-up from team

## Rules

- If you don't know something, say so honestly and offer to connect them with the team
- Never make up booking details, driver names, or prices you aren't sure about
- For urgent issues (missed pickup, safety concern), always say: "I am escalating this to our team right now — someone will contact you within 15 minutes."
- For refund/cancellation requests, tell them to email info@umrahtransport.me with their booking ref
...
- Use occasional relevant Islamic phrases naturally (JazakAllah Khair, InshaAllah, Alhamdulillah) — don't overdo it`
}
...
export async function handleIncomingMessage(
  ...
  // Auto-detect booking reference in message (UT-XXXXX or HUR-XXXXX pattern)
  ...
  if (refMatch) {
    ...
    if (booking) {
      const driverInfo = booking.driverId ? 'A driver has been assigned to your booking.' : 'A driver has not yet been assigned — our team is working on this.'
      bookingContext = `\n\n[BOOKING FOUND: Ref ${booking.reference} | Status: ${booking.status} | Service: ${booking.serviceType} | From: ${booking.pickupLocation} | To: ${booking.dropoffLocation} | Date: ${booking.travelDate} | ${booking.passengers} pax | Vehicle: ${booking.vehicleType} | Amount: £${booking.totalAmount} | Paid: ${booking.paidAt ? 'Yes' : 'No'} | ${driverInfo}]`
    } else {
      bookingContext = `\n\n[No booking found with reference ${refMatch[0]}. Ask the customer to double-check the reference or their email.]`
    }
  } else if (
    ...
    if (bookings.length > 0) {
      bookingContext = `\n\n[BOOKINGS FOUND FOR THIS NUMBER: ${bookings.map(b => `${b.reference} (${b.status}) — ${b.serviceType} on ${b.travelDate}`).join(' | ')}]`
    }
  }

  // Add user message to session history
  const userContent = messageText + bookingContext
  ...
  try {
    ...
    const reply = response.content[0].type === 'text'
      ...
      : 'I am sorry, I could not process that. Please try again or email info@umrahtransport.me'
    ...
    console.error('[WhatsApp Chatbot] Claude API error:', err)
    return 'Assalamu Alaikum! I am having a technical issue right now. Please contact us at info@umrahtransport.me or call +44 7456 938750. JazakAllah Khair!'
  }
}
