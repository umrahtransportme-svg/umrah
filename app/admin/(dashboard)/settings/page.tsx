...
export default function AdminSettingsPage() {
  ...
  // Local form state mirrors store, flushed on Save
  const [form, setForm] = useState(settings)

  // Sync form when tab changes (in case store was updated elsewhere)
            ...
            {[
              ...
              <label key={n.key} className="flex items-center justify-between p-3 border border-slate-100 rounded-xl hover:bg-slate-50 cursor-pointer">
                ...
                <input type="checkbox"
                  ...
                  onChange={(e) => set('notifications', { ...form.notifications, [n.key]: e.target.checked })}
        ...
        <div className="card p-6 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <Field label={`Publishable Key (pk_${form.stripeMode}_…)`}>
              <input type="text" value={form.stripePublishableKey}
                onChange={(e) => set('stripePublishableKey', e.target.value)}
                className="input text-sm font-mono" placeholder={`pk_${form.stripeMode}_...`} />
            </Field>
            <Field label="Secret Key (sk_…)">
              <MaskedInput value={form.stripeSecretKey} onChange={(v) => set('stripeSecretKey', v)}
                show={showSecret} onToggle={() => setShowSecret(!showSecret)} placeholder={`sk_${form.stripeMode}_...`} />
            </Field>
            <Field label="Webhook Signing Secret (whsec_…)">
              <MaskedInput value={form.stripeWebhookSecret} onChange={(v) => set('stripeWebhookSecret', v)}
                show={showWebhook} onToggle={() => setShowWebhook(!showWebhook)} placeholder="whsec_..." />
            </Field>
          </div>
          <div className="flex items-center gap-3">
      ...
      {tab === 'whatsapp' && (
        <div className="card p-6 space-y-5">
          ...
          <div className="p-4 bg-green-50 border border-green-100 rounded-xl text-sm text-green-800">
            Connect the WhatsApp Business API to automatically send booking confirmations, driver updates, and reminders. Get credentials from the <a href="https://developers.facebook.com/docs/whatsapp" target="_blank" rel="noopener noreferrer" className="underline font-medium">Meta Developers Console</a>.
          </div>
          <div className="space-y-4">
            <Field label="Phone Number ID">
              <input type="text" value={form.whatsappPhoneNumberId}
                onChange={(e) => set('whatsappPhoneNumberId', e.target.value)}
                className="input text-sm font-mono" placeholder="1234567890123456" />
            </Field>
            <Field label="WhatsApp Business Account ID">
              <input type="text" value={form.whatsappAccountId}
                onChange={(e) => set('whatsappAccountId', e.target.value)}
                className="input text-sm font-mono" placeholder="1234567890123456" />
            </Field>
            <Field label="Access Token">
              <MaskedInput value={form.whatsappApiToken} onChange={(v) => set('whatsappApiToken', v)}
                show={showWaToken} onToggle={() => setShowWaToken(!showWaToken)} placeholder="EAAxxxxxxxxxx..." />
            </Field>
            <Field label="Display Phone Number (shown to customers)">
              <div className="relative">
                ...
                <input type="text" value={form.whatsappNumber} onChange={(e) => set('whatsappNumber', e.target.value)}
                  className="input pl-9 text-sm" placeholder="+44 7700 000000" />
              </div>
            </Field>
          </div>
          <div className="flex items-center gap-3">
